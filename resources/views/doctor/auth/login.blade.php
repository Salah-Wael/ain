...
@section('content')
            ...
            <form id="formAuthentication" class="mb-3" action="{{ route('doctor.login.store') }}" method="POST">
                @csrf
                <div class="mb-3">
                <label for="id" class="form-label">{{ __('messages.doctor_id') }}</label>
                <input
                    type="text"
                    class="form-control"
                    id="id"
                    name="id"
                    :value="old('id')"
                    placeholder="{{ __('messages.enter_your_id') }}"
                    autofocus
                    />
                ...
                <div class="mb-3 d-flex justify-content-between">
                    ...
                    <a href="{{ route('head.login') }}" class="btn btn-primary w-100 mx-1">{{ __('messages.head_of_department') }}</a>
